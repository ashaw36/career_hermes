...
- **状态: ⚠️ 部分实现** — 后端完整，前端缺入口 (Sprint 2 + Sprint 11 补齐)
- **描述:** 系统根据角色的能力侧重和目标岗位画像，为每条经历计算适配度分数（0–1.0）。
- **验收标准:**
  - AC-013: 适配度计算基于关键词匹配 + 角色权重简单规则 ✅
  - AC-014: 同一条经历在不同角色下可以有不同分数 ✅
  - AC-015: 用户可手动覆盖自动计算的分数 ⚠️ 后端字段 `user_overridden` 已存在且生效，前端无覆盖入口

#### FR-P0-05 简历一键生成
- **状态: ⚠️ 部分实现** — Markdown ✅，PDF 占位 (Sprint 3 + Sprint 11 补齐)
- **描述:** 基于当前激活角色，自动筛选经历、排序、调整描述，输出一份简历。
- **验收标准:**
  - AC-016: 简历自动排除与角色冲突的经历 ✅
  - AC-017: 经历按 Fit Score 降序排列 ✅
  - AC-018: 支持导出 Markdown 和 PDF 格式 ⚠️ Markdown ✅，PDF 占位（生成 `_PDF占位.txt`，未调用 `pdf_exporter.py`）
...
#### FR-P1-05 经历重述引擎
- **状态: ⚠️ 部分实现** — 后端完整，前端缺入口 (Sprint 4 + Sprint 11 补齐)
- **描述:** 同一条经历根据角色侧重自动生成不同描述角度，存储在 `RoleExperienceWeight` 层。
- **验收标准:**
  - AC-037: 自动为每条经历在每个角色下生成重述 ✅
  - AC-038: 用户可以编辑或重置重述 ⚠️ 后端完整（`jd_reframe_engine.py`），前端无编辑/重置入口
  - AC-039: 重述不覆盖原始描述 ✅
...
#### FR-P2-01 智能学习路径推荐
- **状态: ✅ 已实现** (Sprint 5, 9-10)
- **描述:** 基于 Gap 分析结果，推荐公开学习资源（课程、书籍、项目）。
- **验收标准:**
